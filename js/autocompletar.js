...
//var Servidor="autocompletar.php"; // Determina la pagina donde buscar
...
function Solicitud(idContenido,Cadena,Servidor)
{
	...
	if(Cadena && Cadena!=Palabra)
	{
		...
		if(Conexion)
		{
			...
			*	false - Sincronizado */
			...
			Conexion.send("idContenido="+idContenido+"&word="+Cadena+"&"+date.getTime()+'&servidor='+Servidor);
		}else
        {
...
function autocompletar(idContenido,Cadena,Servidor)
{
	...
	if(Cadena.length>=1)
	{
		...
		Solicitud(idContenido,Cadena,Servidor);
	}else{
		document.getElementById(idContenido).style.display="none";
    }
}
...
function selectItem(idContenido,value,Servidor)
{
	...
	autocompletar(idContenido,value,Servidor);
